...
const createFarm = async (req, res) => {
  try {
    ...
    const { name, crop, plantingDate, harvestDate, description, coordinates, area } = req.body;
    
    // Basic validation
    if (!name || !crop || !plantingDate || !harvestDate || !coordinates || !area) {
    ...
    if (!Array.isArray(coordinates) || coordinates.length < 3) {
      const response = new ResponseEntity(0, "Invalid coordinates. Must be an array with at least 3 points", {});
      return res.status(400).json(response);
    }
    ...
    if (plantDate >= harvestDate_) {
      const response = new ResponseEntity(0, "Harvest date must be after planting date", {});
      return res.status(400).json(response);
    }
...
const updateFarm = async (req, res) => {
  try {
    ...
    const response = new ResponseEntity(1, "Farm updated successfully", updatedFarm);
    res.status(200).json(response);
  } catch (error) {
